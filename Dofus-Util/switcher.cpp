// Dofus Switcher.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
        ...
        if (msg.message == WM_HOTKEY)
        {
            for (auto bind : binds) {
                if (bind.first == msg.wParam) {
                    //std::cout << "Bringing " << bind.second << " to the front!" << std::endl;
...
void Switcher::setupAccount() {
    ...
    do {
        ...
        //GetKeyNameTextW(MapVirtualKeyW(key, MAPVK_VK_TO_CHAR) << 16, xdd, sizeof(xdd) / sizeof(WCHAR));
    ...
    //RegisterHotKey(nullptr, count, 0, keybind);
